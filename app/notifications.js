...
router.post("", async (req, res) => {
    ...
    user = await User.findById(userID)
        .exec()
        .catch((err) => {
            res.status(400).send("Error in user quering.");
            interrupt = true;
        });

    if (!user) {
        if (!interrupt) res.status(404).json({ error: "User does not exist" });
        return;
    }
...
router.get("/:id", async (req, res) => {
    ...
    let notification = await Notification.findById(req.params.id)
        .exec()
        .catch(() => {
            res.status(400).send("Error in Notification quering (Id may be wrong)");
            interrupt = true;
        });
