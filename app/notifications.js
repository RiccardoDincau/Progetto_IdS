...
router.post("", async (req, res) => {
    ...
    let report = null;
    report = await Report.findById(reportID).exec().catch((err) => {
    ...
    res.location(req.path + notificationId).status(201).send();
});

router.get("/:id", async (req, res) => {
    let notification = await Notification.findById(req.params.id).exec().catch(() => {
